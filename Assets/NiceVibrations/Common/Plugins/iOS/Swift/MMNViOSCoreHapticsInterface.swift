...
@objc public static func UpdateContinuousHaptic(intensity: Float, sharpness: Float, threaded: Bool)
{
    if (threaded)
    {
        let queue = DispatchQueue(label: QueueID, qos: .userInitiated);
        queue.async {
            UpdateContinuousHapticThread(intensity:intensity, sharpness:sharpness);
        }
    }
    else
    {
  ...
  @objc public static func UpdateContinuousHapticThread(intensity: Float, sharpness: Float)
  {
    if (!CoreHapticsSupported())
    {
        return;
